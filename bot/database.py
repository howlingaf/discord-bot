...
def db_init():
    with _db() as conn:
        ...
        # ---- Reaction tracking (who reacted to a given post) ----
        conn.execute("""
        CREATE TABLE IF NOT EXISTS reaction_tracks (
          message_id        INTEGER PRIMARY KEY,
          channel_id        INTEGER NOT NULL,
          emoji             TEXT NOT NULL,
          roster_channel_id INTEGER NOT NULL,
        ...
        conn.execute("""
        CREATE TABLE IF NOT EXISTS reaction_reactors (
...
# ---- Reaction tracking helpers ----

def reaction_track_save(message_id: int, channel_id: int, emoji: str,
                        roster_channel_id: int, roster_message_id: int | None):
    now = int(time.time())
    with _db() as conn:
        conn.execute(
            """INSERT INTO reaction_tracks(message_id, channel_id, emoji, roster_channel_id, roster_message_id, created_at)
               VALUES(?,?,?,?,?,?)
               ON CONFLICT(message_id) DO UPDATE SET
                 channel_id=excluded.channel_id,
                 emoji=excluded.emoji,
                 roster_channel_id=excluded.roster_channel_id,
                 roster_message_id=excluded.roster_message_id""",
            (message_id, channel_id, emoji, roster_channel_id, roster_message_id, now),
        )
        conn.commit()


def reaction_track_set_roster_message(message_id: int, roster_message_id: int):
    with _db() as conn:
        conn.execute(
            "UPDATE reaction_tracks SET roster_message_id=? WHERE message_id=?",
            (roster_message_id, message_id),
        )
        conn.commit()


def _track_row(row) -> dict:
    return {
        "message_id": row[0], "channel_id": row[1], "emoji": row[2],
        "roster_channel_id": row[3], "roster_message_id": row[4], "created_at": row[5],
    }


def reaction_track_get(message_id: int) -> dict | None:
    with _db() as conn:
        row = conn.execute(
            "SELECT message_id, channel_id, emoji, roster_channel_id, roster_message_id, created_at "
            "FROM reaction_tracks WHERE message_id=?",
            (message_id,),
        ).fetchone()
        return _track_row(row) if row else None


def reaction_tracks_all() -> list[dict]:
    with _db() as conn:
        rows = conn.execute(
            "SELECT message_id, channel_id, emoji, roster_channel_id, roster_message_id, created_at "
            "FROM reaction_tracks"
        ).fetchall()
        return [_track_row(r) for r in rows]


def reaction_track_delete(message_id: int) -> bool:
    with _db() as conn:
        cur = conn.execute("DELETE FROM reaction_tracks WHERE message_id=?", (message_id,))
        conn.execute("DELETE FROM reaction_reactors WHERE message_id=?", (message_id,))
        conn.commit()
        return cur.rowcount > 0


def reaction_reactor_add(message_id: int, user_id: int) -> bool:
    """Record a reactor. Returns True if newly added (keeps the original reacted_at)."""
    now = int(time.time())
    with _db() as conn:
        cur = conn.execute(
            "INSERT OR IGNORE INTO reaction_reactors(message_id, user_id, reacted_at) VALUES(?,?,?)",
...
def reaction_reactor_remove(message_id: int, user_id: int) -> bool:
    with _db() as conn:
        cur = conn.execute(
            "DELETE FROM reaction_reactors WHERE message_id=? AND user_id=?",
            (message_id, user_id),
        )
        conn.commit()
        return cur.rowcount > 0
...
def reaction_reactors_get(message_id: int) -> list[dict]:
    with _db() as conn:
        rows = conn.execute(
            "SELECT user_id, reacted_at FROM reaction_reactors WHERE message_id=? ORDER BY reacted_at, user_id",
            (message_id,),
        ).fetchall()
        return [{"user_id": r[0], "reacted_at": r[1]} for r in rows]


def reaction_reactors_sync(message_id: int, user_ids: list[int]) -> tuple[int, int]:
    """Reconcile stored reactors against the live truth from Discord.

    Adds unseen users (stamped now) and drops any who un-reacted while the bot
    was down. Existing rows keep their original reacted_at. Returns (added, removed).
    """
    now = int(time.time())
    with _db() as conn:
        existing = {r[0] for r in conn.execute(
            "SELECT user_id FROM reaction_reactors WHERE message_id=?", (message_id,)
        ).fetchall()}
        live = set(user_ids)
        ...
        if added:
            conn.executemany(
                "INSERT OR IGNORE INTO reaction_reactors(message_id, user_id, reacted_at) VALUES(?,?,?)",
                [(message_id, uid, now) for uid in added],
            )
        if removed:
            conn.executemany(
                "DELETE FROM reaction_reactors WHERE message_id=? AND user_id=?",
                [(message_id, uid) for uid in removed],
            )
        conn.commit()
